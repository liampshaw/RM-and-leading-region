...
Make a plot of these results, including information on PTU pangenome size:

```{r}
ptu_for_plotting = ptu %>% pivot_longer(cols=c("n.core", "n.others"))
ptu_for_plotting$name = ordered(ptu_for_plotting$name,
                                levels=c("n.others", "n.core"))
ggplot(ptu_for_plotting, aes(PTU, value, fill=name))+
  geom_bar(stat="identity", position="stack")+
  coord_flip()
# Want average size of the PTU as well...
...
colnames(results.df)[2:ncol(results.df)] = names(table(lr.no.duplicates.subset$PTU))
# Normalise scores by the number of plasmids within PTU?
n.plasmids = table(lr.no.duplicates.subset$PTU)
...
all_rm_targets = read.csv('../data/all_rebase_targets_250131_6bp_less_deambiguated.txt', header=F, sep=' ')
all_rm_targets_4bp =all_rm_targets$V1[all_rm_targets$V2==4]
...
chisq.test(matrix(c(3, 10, 13, 230 ), nrow=2))
chisq.test(matrix(c(4, 9, 99, 144 ), nrow=2))
...
arrange(median)
# Take the top 1% depleted
results.df.norm.average.top.depleted = results.df.norm.average[which(results.df.norm.average$median< quantile(results.df.norm.average$median, probs = c(0.01))),] # Top 5% since so few
all_rm_targets_5bp =all_rm_targets$V1[all_rm_targets$V2==5]
results.df.norm.average.top.depleted$target = ifelse(results.df.norm.average.top.depleted$kmer %in%
all_rm_targets_5bp,
"target", "non-target")
table(results.df.norm.average.top.depleted$target)
chisq.test(matrix(c(5, 6, 277, 736 ), nrow=2))
...
# Read in all RM targets
all_rm_targets = read.csv('../data/all_rebase_targets_250131_6bp_less_deambiguated.txt', header=F, sep=' ')
all_rm_targets_6bp =all_rm_targets$V1[all_rm_targets$V2==6]
results.df.norm.average.top.depleted$target = ifelse(results.df.norm.average.top.depleted$kmer %in%
all_rm_targets_6bp,
"target", "non-target")
table(results.df.norm.average.top.depleted$palindrome)
...
```{r}
core_k6 = read.csv('~/Downloads/trieste/one_per_family/all_core_k6.csv')
accessory_k6 = read.csv('~/Downloads/trieste/one_per_family/all_accessory_k6.csv')
merge_k6 = merge(core_k6, accessory_k6, by="word")
merge_k6$diff = merge_k6$score.y-merge_k6$score.y
merge_k6$palindrome = ifelse(merge_k6$word %in% palindromes,
                             "palindrome", "other")
ggplot(merge_k6, aes(score.x-score.y))+
  geom_histogram()+

...
Repeated this analysis with one gene representative per family:
...
### All hard shell vs. accessory, one representative per gene family

We subsampled to one representative per gene family and the same number of hard shell and accessory genes, whichever was smaller (PTU-Bac37 was the only one that had more hard shell than accessory genes). 
```{r}
...
ggsave(p.lines.expectation, file='../results/Figure-S2-6mer-deviation-expectation.pdf',
       width=10, height=6)
# Plot only first 50kb
...
```
```{r}
# Sliding window for GC content
p.lines.GC = ggplot(d.reduced.median.plot, aes(x, GC/5000 * 100))+
  geom_hline(yintercept = 0, linetype='dashed')+
  theme_bw()+
  geom_path(aes(group=PTU, alpha=normalized_n))+
  facet_wrap(~PTU, scales="free_x", nrow=3)+
  ...
  xlab("Position on plasmid (kb)")+
  theme(legend.position = c(0.8, 0.2))
ggsave(p.lines.GC, file='../results/Figure-S1-GC-content.pdf',
       width=10, height=6)


# And for palindrome density (not expected - actual values)
p.lines.density = ggplot(d.reduced.median.plot, aes(x, median.original*1000))+
  geom_hline(yintercept = 0, linetype='dashed')+
  theme_bw()+
  geom_path(aes(group=PTU, alpha=normalized_n))+
  facet_wrap(~PTU, scales="free_x", nrow=3)+
  ...
  xlab("Position on plasmid (kb)")+
  theme(legend.position = c(0.8, 0.2))
ggsave(p.lines.density, file='../results/Figure-S3-6mer-palindrome-density.pdf',
       width=10, height=6)
...
diff.PTU.averages = diff.leading.region.average %>% group_by(PTU) %>%
  summarise(mean.GC=mean(diff.GC),
            ...
            n=length(diff.GC))
ggplot(diff.PTU.averages, aes(mean.GC, mean.palindrome.diff))+
  geom_hline(yintercept = 0, linetype='dashed')+
  ggrepel::geom_text_repel(aes(label=PTU))+
    geom_point(aes(size=n), shape=21, colour="black", fill="grey")+
  theme_bw()+
  theme(panel.grid = element_blank())+
  xlab("GC difference between first 5kb\nand plasmid average (%)")+
  ylab("Additional palindrome depletion in leading region (per kb)")


# Calculate actual GC and actual palindrome density
# for a given x-value. I think 5kb is appropriate for leading region to 
# get rid of start effects. 
...
ggplot(d.GC.palindrome, aes(diff.GC, density.depletion))+
  geom_point()
...
ggplot(d.GC.palindrome, aes(diff.GC, density.depletion, fill=PTU))+
  geom_point(shape=21, colour="black")+
  facet_wrap(~PTU)
p.gc.correlation = ggplot(d.GC.palindrome, aes(diff.GC, density.depletion))+
  geom_point()+
  theme_bw()+
  ...
  ylab("Relative palindrome depletion at 5-10kb (per kb)")+
    xlim(c(-15, 10))+
  ylim(-13, 14)
p.gc.correlation.ptu =  ggplot(d.GC.palindrome.PTU, aes(diff.GC, density.depletion))+
...
cowplot::plot_grid(p.gc.correlation+ggtitle("(a)"), p.gc.correlation.ptu+ggtitle("(b)"))
dev.off()
...
summary(lm(density.depletion ~ diff.GC*PTU, data=d.GC.palindrome))
...
overall.median.GC = d.reduced %>% filter(x<50000) %>%
  ...
  summarise(y=mean(GC_n),
            n=length(GC_n))
p.overall.average.GC = ggplot(overall.median.GC, aes(x, y/5000 * 100))+
  geom_line()+
  theme_bw()+
  theme(panel.grid = element_blank())+
  ...
  xlab("Position on plasmid (kb)")+
  theme(axis.text=element_text(colour="black"))
p.overall.average.GC
# Save
ggsave(p.overall.average.GC, file='../results/Figure-S1-GC-mean-overall.pdf', 
       width=5, height=2)
```
...
ggsave(scores.plot, file='../results/Figure-S4-6mer-palindromes-scores.pdf',
       height=10, width=10)
```
...
p.depletion.targets = ggplot(results, aes(median.score, as.factor(PTU)))+
  ...
  theme(axis.text=element_text(colour="black", size=12),
        ...
        axis.text.x=element_text(size=18))+
  theme(axis.text.y=element_text(hjust=0))
ggsave(p.depletion.targets, file='../results/Figure-4-figure-depletion-within-range-targets.pdf',
       width=10, height=6)
